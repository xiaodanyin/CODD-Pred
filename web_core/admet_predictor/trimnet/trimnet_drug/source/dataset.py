...
try:
    from utils import onehot_encoding_unk, scaffold_randomized_spliting, onehot_encoding, split_multi_label_containNan
except:
    from trimnet.trimnet_drug.source.utils import onehot_encoding_unk, scaffold_randomized_spliting, onehot_encoding, split_multi_label_containNan
# import sys
# sys.path.append('./')
# from utils import onehot_encoding_unk, scaffold_randomized_spliting, onehot_encoding, split_multi_label_containNan
...
class NanDataset(InMemoryDataset):
    ...
    def __init__(self, root, dataset, tasks, mode, seed, transform=None, pre_transform=None, pre_filter=None):
        self.tasks = tasks
        self.dataset = dataset
        self.mode = mode
        self.seed = seed
        self.weights = 0
        super(NanDataset, self).__init__(root, transform, pre_transform, pre_filter)
        if self.mode == 'train':
            self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    @property
    def processed_file_names(self):
        return ['{}_train_{}.pt'.format(self.dataset, self.seed),
    ...
    def process(self):
        ...
        trn_df, val_df, test_df, weights = split_multi_label_containNan(df, self.tasks, self.seed)
